...
public class CommandProcessor {
	...
	public static void runAlgorithm(String algorithmName,
		...
		//
			...
			algo.runAlgorithm(sd, true, false, outputFile, outputSeqIdentifiers);
			System.out.println(algo.printStatistics());
		}else if ("USpan".equals(algorithmName)) {
			...
			algoAgrawal.runAlgorithm(patterns, outputFile, database.size(),
					minconf);
			algoAgrawal.printStats();
		} else if ("Closed_association_rules(using_fpclose)".equals(algorithmName)) {
			double minsup = getParamAsDouble(parameters[0]);
			double minconf = getParamAsDouble(parameters[1]);
			...
			// STEP 2: Generate all rules from the set of frequent itemsets
			// (based on Agrawal & Srikant, 94)
			AlgoClosedRules_UsingFPClose algoAgrawal = new AlgoClosedRules_UsingFPClose();
		...
		// algo.printStats();
			...
			algo.runAlgorithm(inputFile, outputFile, minutil);
			algo.printStats();
		} else if ("FHMFreq".equals(algorithmName)) {
			int minutil = getParamAsInteger(parameters[0]);
			...
			algo.printStats();
		}else if ("FCHM".equals(algorithmName)) {
			int minutil = getParamAsInteger(parameters[0]);
			double minbond = getParamAsDouble(parameters[1]);
			// Applying the algorithm
			...
			algo.printStats();
		}else if ("HUG-Miner".equals(algorithmName)) {
			int minutil = getParamAsInteger(parameters[0]);
			// Applying the algorithm
			AlgoHUGMiner hugMiner = new AlgoHUGMiner();
			hugMiner.runAlgorithm(inputFile, outputFile, minutil);
			hugMiner.printStats();
		}else if ("CHUI-Miner".equals(algorithmName)) {
			int minutil = getParamAsInteger(parameters[0]);
			// Applying the algorithm
			AlgoCHUIMiner algo = new AlgoCHUIMiner(true);
			algo.runAlgorithm(inputFile, minutil, outputFile);
			algo.printStats();
		}else if ("GHUI-Miner".equals(algorithmName)) {
			int minutil = getParamAsInteger(parameters[0]);
		...
		}else if ("FOSHU".equals(algorithmName)) {
			...
			algo.printStats();
		}else if ("TS-HOUN".equals(algorithmName)) {
			double min_utility_ratio = getParamAsDouble(parameters[0]);
			...
			algo.printStats();
		}else if ("HUSRM".equals(algorithmName)) {
			int minutil = getParamAsInteger(parameters[0]);
			double minconf = getParamAsDouble(parameters[1]);
